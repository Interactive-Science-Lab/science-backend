...
function registerEmail(user) {
  ...
  const link = `${siteSettings.siteLink}/users/verify/${user.user_id}/${encodeURIComponent(user_hash)}` //This should be a front end link
  const text = `Thank you for registering for ${siteSettings.siteNames}. You can copy & paste this link to complete the verification process: ${link}`
  const html = verifyEmailTemplate(link)
  sendEmail(user, `Thank You For Registering With ${siteSettings.siteNames}`, text, html)
  return user_hash
}

function forgotPasswordEmail(user) {
  const user_hash = forgotPasswordHash(user)
  const link = `${siteSettings.siteLink}/users/resetPassword/${user.username}/${encodeURIComponent(user_hash)}` //This should be a front end link
...
const sendEmail = (user, subject, text, html) => {
  let transporter = nodeMailer.createTransport({
    ...
    secure: true,
    auth: {
      user: siteSettings.siteEmail,
      pass: siteSettings.secretEmail
    }
  });
  let mailOptions = {
    to: user.user_email,
